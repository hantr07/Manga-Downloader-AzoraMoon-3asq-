#!/usr/bin/env python3
import sys, os, re, argparse, asyncio, requests
...
async def fetch_chapter_images(chapter_url):
    ...
    imgs = [
        img.get("data-src", img.get("src")).strip()
...
def download_images(imgs, chapter_url, outdir):
    ...
    for i, url in enumerate(imgs, 1):
        ...
        if os.path.exists(fn):
            print(f"[{i}/{len(imgs)}] Skipping {fn}")
            continue
...
# -------- Series Chapter Links -------- #
async def fetch_chapter_links(series_url):
    html = await fetch_html(series_url)
    soup = BeautifulSoup(html, "html.parser")

    # Manga title for folder name
    title = soup.select_one("h1") or soup.select_one("div.post-title h1")
    manga_title = title.text.strip().replace(" ", "_") if title else "Manga"

    chapters = [a["href"].strip() for a in soup.select("ul.main.version-chap li a") if a.get("href")]
    chapters = chapters[::-1]  # oldest to newest
...
async def main():
    ...
    if re.search(r"/manga/[^/]+/$", url):  # series page
